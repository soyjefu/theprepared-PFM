...
class TransactionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {"is_repayment": "이 거래를 부채상환으로 처리합니다."}
...
class AccountForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if account_type == "순자산" and category != "GENERAL":
            self.add_error(
                "category", "순자산 타입은 '일반' 유형만 선택할 수 있습니다."
            )

        elif account_type == "자산" and category not in [
            "GENERAL",
            "VARIABLE",
            "SAVING",
        ]:
...
class TransactionPresetForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        preset_type = None
        if self.data:
            preset_type = self.data.get("preset_type")
        if not preset_type and self.instance:
            preset_type = getattr(self.instance, "preset_type", None)
        if not preset_type and self.initial:
            preset_type = self.initial.get("preset_type")

        if preset_type == "FREQUENT":
            self.fields["amount"].required = False
            self.fields["day_of_month"].required = False
        elif preset_type == "FIXED":
            self.fields["amount"].required = True
            self.fields["amount"].error_messages["required"] = "고정항목은 금액을 필수로 입력해야 합니다."
            self.fields["day_of_month"].required = True
            self.fields["day_of_month"].error_messages["required"] = "고정항목은 고정 일자를 필수로 입력해야 합니다."
    ...
    def clean(self):
        ...
        if preset_type == "FIXED":
            if day_of_month is not None and not (1 <= day_of_month <= 31):
                self.add_error(
                    "day_of_month", "고정 일자는 1에서 31 사이의 숫자여야 합니다."
                )
        return cleaned_data
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            self.fields["account"].queryset = Account.objects.filter(
                owner=user, type="비용", is_active=True
            )
