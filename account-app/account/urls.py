...
urlpatterns = [
    ...
    # 가계부 기능 URL (접두사 없이)
    path(
        "transaction/new/", views.transaction_create, name="transaction_create"
    ),  # 거래입력을 첫 번째로
    ...
    path(
        "transaction/<int:pk>/inline-update/",
        views.transaction_inline_update,
        name="transaction_inline_update",
    ),
    path(
    ...
    path(
        "account/<int:pk>/investment-principal/",
        views.investment_principal_update,
        name="investment_principal_update",
    ),
    # 예산 및 통계계 관련
