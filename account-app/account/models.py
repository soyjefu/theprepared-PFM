...
class TransactionPreset(models.Model):
    ...
    preset_type = models.CharField(
        max_length=10, choices=PRESET_TYPES, verbose_name="프리셋 타입"
    )
    ...
    amount = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name="금액 (자주입력은 비워둘 수 있음)",
    )
    debit_account = models.ForeignKey(
        Account,
        related_name="debit_presets",
        on_delete=models.CASCADE,
        verbose_name="차변 계정",
    )
    credit_account = models.ForeignKey(
        Account,
        related_name="credit_presets",
        on_delete=models.CASCADE,
        verbose_name="대변 계정",
    )
    owner = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="소유자")
    ...
    day_of_month = models.IntegerField(
        null=True, blank=True, verbose_name="고정 일자 (예: 25)"
    )
...
class Budget(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=12, decimal_places=0, verbose_name="예산 금액"
    )
